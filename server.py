...
from src.mesh_processor import get_candidate_nodes, get_story_levels, get_mesh_bounds
...
def evaluate():
    ...
    Steps:
      1. Extract mesh geometry — candidate nodes via mesh_processor (crease + floor boundary)
      2. Compute mesh descriptors + structural system selection (rules bridge)
      3. Gemini → topology only (nodes + edges, no sections)
      4. Section sizer → deterministic R19/R20/R28 sizing
      5. PyNite FEA + code checks (R39/R42)
      6. Section upgrade if checks fail (≤ max_iterations, no extra Gemini call)
      7. Section scaling → MIN_COST / BALANCED / MIN_DISP variants + FEA on each
      8. Return all 3 variants
    """
    try:
        ...
        # Phase 1: candidate structural nodes from mesh_processor
        candidate_nodes, crease_edges = get_candidate_nodes(
            ge,
            max_count=80,
            crease_angle_deg=15.0,
        )
        story_levels = get_story_levels(ge, floor_height_m=4.0)
        bounds       = get_mesh_bounds(ge)
        peak_points  = ge.get_max_height_points()

        print(
            f"Mesh ({mesh_path}): {len(candidate_nodes)} candidate nodes, "
            f"{len(crease_edges)} crease edges, {len(story_levels)} story levels."
        )
        ...
        if mesh_desc:
            print(
                f"System: {mesh_desc.get('lateral_system')} | "
                f"H/W={mesh_desc.get('H_W_ratio', 0):.2f} | "
                f"{mesh_desc.get('aspect_category')}"
            )

        # ── STEP 3+: AI topology → section sizing → FEA loop → variants ───────
        base_geom = {
            "primary_nodes":  candidate_nodes,
            "primary_edges":  crease_edges,
        ...
        # max_iterations=1 to stay within 60 s HF proxy timeout.
        ...
        if not base_graph or base_graph.number_of_nodes() == 0:
            return jsonify({'error': 'Pipeline produced no structural nodes.'}), 500

        # ── STEP 7: 3 variants via section scaling + FEA ───────────────────────
